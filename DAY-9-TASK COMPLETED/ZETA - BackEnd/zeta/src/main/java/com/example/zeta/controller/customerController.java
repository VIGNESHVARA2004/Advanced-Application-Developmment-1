...
public class customerController {
    ...
    public ResponseEntity<String> editRecharge(@PathVariable Long rechargeId, @RequestBody RechargeData rechargeData) {
        CustomerService.editRecharge(rechargeId, rechargeData);
        return ResponseEntity.ok("Recharge Edited");
    }

    @DeleteMapping("/recharge/{rechargeId}")
    public ResponseEntity<String> deleteRecharge(@PathVariable Long rechargeId) {
        CustomerService.deleteRecharge(rechargeId);
